var functionsCondensedMain = {
    ...
    // --- Generic CPU Tracking Helpers ---

// --- Generic CPU Tracking Helpers ---
startTracking: function startTracking(statName) {
    if (!Memory.cpuStats) Memory.cpuStats = {};
    if (!Memory.cpuStats[statName]) {
        Memory.cpuStats[statName] = { total: 0, runs: 0, average: 0 };
    }
    Memory.cpuStats[statName]._start = Game.cpu.getUsed(); // temporary marker
},
...
endTracking: function endTracking(statName) {
    ...
    if (controller.level >= 4) {
       ...
       // If there's no controller link, check for construction sites and create one if none exists
       if (!controllerLink) {
          // Check for existing construction sites for links within range 1
          const existingSite = room.find(FIND_CONSTRUCTION_SITES, {
             filter: s => s.structureType === STRUCTURE_LINK && s.pos.inRangeTo(controller.pos, 1)
          })[0];
          ...
          if (!existingSite) {
             ...
             for (let o of offsets) {
                ...
                // ensure position is walkable, has no structure, no hostile construction, and not the controller tile
                const roomTerrain = room.getTerrain().get(px, py);
                if (roomTerrain === TERRAIN_MASK_WALL) continue;

                const blockingStructures = pos.lookFor(LOOK_STRUCTURES)
                    .filter(s => s.structureType !== STRUCTURE_ROAD);

                if (blockingStructures.length > 0) continue;
                ...
                if (res === OK) {
                    // log optionally
                    // console.log(`Created link construction site at ${pos} for room ${roomName}`);
                    break;
                }
             }
          }
       ...
       // If links are up and we previously created a temp container, remove it so it can be used elsewhere
       if (Memory.rooms[roomName].LinkUp && Memory.rooms[roomName].tempControllerContainer) {
          ...
          // If we stored a position, remove construction sites there and destroy built container if present
          if (tmp.pos) {
             const pos = new RoomPosition(tmp.pos.x, tmp.pos.y, roomName);
             const sites = pos.lookFor(LOOK_CONSTRUCTION_SITES).filter(s => s.structureType === STRUCTURE_CONTAINER);
             for (const s of sites) {
                try { s.remove(); } catch (e) {}
             }
             const structs = pos.lookFor(LOOK_STRUCTURES).filter(s => s.structureType === STRUCTURE_CONTAINER);
       ...
       // below RCL4: still populate memory if links already exist, but do not attempt to create one
       const links = room.find(FIND_STRUCTURES, {
          filter: s => s.structureType === STRUCTURE_LINK
       });

       const controllerLink = links.find(link => link.pos.inRangeTo(controller.pos, 1));
       const storageLink = room.storage ? links.find(link => link.pos.inRangeTo(room.storage.pos, 3)) : null;
       ...
       // If links not available, attempt to place a temporary container near the controller (so creeps can drop energy).
       // We'll mark it in memory so it can be removed later when the link is built.
       if (!Memory.rooms[roomName].LinkUp) {
          ...
          const existingContainer = room.find(FIND_STRUCTURES, {
             filter: s => s.structureType === STRUCTURE_CONTAINER && s.pos.inRangeTo(controller.pos, 1)
          })[0];

          // If there's already a container, record it but don't destroy it later unless we created it
          if (existingContainer) {
             ...
             if (!existingSite && !Memory.rooms[roomName].tempControllerContainer) {
                ...
                for (let o of offsets) {
                    ...
                    if (res === OK) {
                       // mark in memory that we made a temporary controller container and store its pos
                       Memory.rooms[roomName].tempControllerContainer = { pos: { x: px, y: py } , createdTick: Game.time };
                       break;
                    }
                }
                ...
                Memory.rooms[roomName].tempControllerContainer = { pos: { x: existingSite.pos.x, y: existingSite.pos.y }, createdTick: Game.time };
             }
          }
       } else {
          // If LinkUp true but a temp container exists (edge case), remove it so the container can be reused
...
updateRoomSources: function updateRoomSources(spawn) {
    ...
    sources.forEach(source => {
        ...
        if (containers.length > 0) {
            ...
            const pathControler = PathFinder.search(spawn.room.controller.pos, { pos: container.pos, range: 1 });
...
findBaseLocation: function findBaseLocation(room) {
  ...
  if (!Memory.rooms[room.name].Mineral) Memory.rooms[room.name].Mineral = room.find(FIND_MINERALS).length ? room.find(FIND_MINERALS)[0].mineralType : "NONE";
  // If this room was already checked earlier, skip re-checking
  if (Memory.rooms[room.name].BaseChecked) return;
  ...
  // edgeMargin is how many tiles we want to keep free from each wall
  // Set to 1 so the base will not touch the outer room border or door tiles
  const edgeMargin = 2;

  // top-left of the 13x13 block must be between minTopLeft and maxTopLeft inclusive
  // These values guarantee the entire 13x13 region is inside the room and away from edges
  ...
  // Derive a starting top-left by centering the 13x13 block around the average source position
  // Clamp the start to the valid top-left range so BFS begins inside the allowed rectangle
  const startTopLeftX = Math.min(
    Math.max(Math.round(avgX) - Math.floor(size / 2), minTopLeft),
    maxTopLeft
  );
  const startTopLeftY = Math.min(
    Math.max(Math.round(avgY) - Math.floor(size / 2), minTopLeft),
    maxTopLeft
  );

  // Breadth-first search (queue) over candidate top-left coordinates
  let queue = [[startTopLeftX, startTopLeftY]];
  ...
  while (queue.length > 0) {
    const [x, y] = queue.shift(); // pop the next candidate
    const key = `${x},${y}`;
    ...
    if (x < minTopLeft || x > maxTopLeft || y < minTopLeft || y > maxTopLeft) continue;

    // Assume the candidate block is valid until we find a blocking tile
    let isValid = true;

    // Check every tile inside the size x size block
    // terrain.get returns 0 for plain, 1 for wall, 2 for swamp
    for (let ox = 0; ox < size; ox++) {
      ...
      if (!isValid) break; // stop checking this block early if invalid
    }
    ...
    // Otherwise enqueue orthogonal neighbors (up/down/left/right) inside the valid rectangle
    const neighbors = [
      [x + 1, y],
      [x - 1, y],
      [x, y + 1],
  ...
  // If a base center was found, save it in Memory, add to claim list, and place a flag
  if (basePosition) {
    Memory.rooms[room.name].BaseCompatible = basePosition;
    if (!Memory.roomToClaim.includes(room.name)) Memory.roomToClaim.push(room.name);
    room.createFlag(basePosition.x, basePosition.y, `C.${room.name}`);
  } else {
    // Otherwise record that the room has no compatible base placement under these rules
    Memory.rooms[room.name].BaseCompatible = false;
  }
},
...
tagRenewablesForRoom: function tagRenewablesForRoom(room) {
    ...
    if (!roomMem || !roomMem.TargetScreep) return;
